...
const Profile = () => {
  ...
  // 🔥 新增：保存用户选择的真实文件对象 (File)，用于发给后端的 /api/upload 接口
  ...
  const uploadImage = async (file) => {
    ...
    formData.append('image', file); // 'image' 要和后端 multer 配置的名字一致
    
    // 发送到我们即将写的后端 /api/upload 接口
    const res = await axios.post('/api/upload', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
    return res.data.url; // 返回 Cloudinary 给出的云端链接
  };
  ...
  return (
    <div className="w-full min-h-screen pb-24 overflow-x-hidden text-white relative">
      ...
      {/* --- 1. Banner 区域 --- */}
      <div className="relative h-[25vh] md:h-[35vh] w-full overflow-hidden bg-gray-900 group">
        <img 
          src={isEditing ? editData.bannerUrl : (profile.bannerUrl || '/assets/bg.png')} 
          alt="Profile Banner" 
          className={`w-full h-full object-cover transition-all duration-500 ${isEditing ? 'opacity-40 blur-sm scale-105' : 'opacity-60'}`}
        />
        ...
        {isOwnProfile && isEditing && (
           <button 
             ...
             className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/80 backdrop-blur-md px-6 py-3 rounded-full border border-white/30 transition-all flex items-center gap-2 z-30 font-bold tracking-widest text-sm"
           >
             <FaCamera /> 更换封面图
           </button>
        )}
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 -mt-12 md:-mt-20 relative z-10">
        <div className="flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-8 text-center md:text-left">
            ...
            <div className="w-24 h-24 md:w-40 md:h-40 rounded-2xl md:rounded-3xl overflow-hidden border-4 border-black bg-gray-900 shadow-2xl relative">
              ...
              {isOwnProfile && isEditing && (
                <div 
                  ...
                  className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center cursor-pointer opacity-100 transition-opacity"
                >
          ...
          <div className="pb-2 md:pb-4 w-full md:w-auto flex justify-center">
            {isOwnProfile ? (
              !isEditing ? (
                <button onClick={() => setIsEditing(true)} className="px-6 py-3 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 rounded-full font-bold transition-all flex items-center gap-2 text-sm md:text-base w-full md:w-auto justify-center shadow-lg">
                  <FaUserEdit /> 编辑资料
                </button>
              ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mt-8 md:mt-12">
          ...
          <div className="md:col-span-2 space-y-6 md:space-y-8">
            ...
            <motion.div 
              ...
              className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-8 shadow-xl"
            >
              ...
              {isEditing ? (
                <div className="relative">
                  ...
                  />
                  <div className="absolute right-2 bottom-2 text-xs text-gray-500">支持 BBCode</div>
                </div>
              ) : (
